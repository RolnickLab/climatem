...
class ClimateDataset(torch.utils.data.Dataset):
    ...
    def load_coordinates_into_mem(self, paths: List[List[str]]) -> np.ndarray:
        ...
        if paths[0][0][-5:] == ".grib":
            ...
            print("Mappings dir:", MAPPINGS_DIR)
    ...
    def __init__(  # inherits all the stuff from Base
        ...
        **kwargs,
        ...
        if os.path.isfile(os.path.join(output_save_dir, fname)):  # we first need to get the name here to test that...
            ...
            # save self.Data
            self.Data = self.raw_data  # this is the raw data, not normalised
            # just save this in scratch/data
            # np.save("/home/mila/s/sebastian.hickman/work/new_climatem/climatem/notebooks/RAW_DATA.npy", self.Data)

            self.Data = self.normalize_data(self.raw_data, stats)
            # np.save("/home/mila/s/sebastian.hickman/work/new_climatem/climatem/notebooks/NORMALISED_DATA.npy", self.Data)

            if self.seasonality_removal:
                self.Data = self.remove_seasonality(self.Data)
                # np.save("/home/mila/s/sebastian.hickman/work/new_climatem/climatem/notebooks/NORMALISED_DESEASONALISED_DATA.npy", self.Data)
