...
class ClimateDataset(torch.utils.data.Dataset):
        ...
        def load_into_mem(self, paths: List[List[str]], num_vars: int, channels_last=True, seq_to_seq=True): #-> np.ndarray():
            """
        ...
        def load_coordinates_into_mem(self, paths: List[List[str]]) -> np.ndarray:
            ...
            if paths[0][0][-5:] == '.grib':
                ...
                for vlist in [paths[0]]:
                    ...
                    print("length_paths_list", len(vlist))
                    temp_data = xr.open_mfdataset(vlist, concat_dim='time',
                                                combine='nested').compute()  # .compute is not necessary but eh, doesn't hurt
                    print('self.in_variables:', self.in_variables)
                    #NOTE:(seb) - should this be for all possible variables? Not sure...
                    if "tas" in self.in_variables or "pr" in self.in_variables or 'psl' in self.in_variables or "ts" in self.in_variables:
        ...
        def get_causal_data(self, tau, channels_last, num_vars, num_scenarios, num_ensembles, num_years, mode, 
                            num_months_aggregated=1, ratio_train=None, interval_length=100):
            """
            ...
            if isinstance(num_months_aggregated, (int, np.integer)) and num_months_aggregated>1:
                ...
                if mode == 'train' or mode == 'train+val':
                    ...
                    # z-score normalization
                    stats_fname, coordinates_fname = self.get_save_name_from_kwargs(mode="train+val", file='statistics', kwargs=self.fname_kwargs, causal=True)
        ...
        def get_save_name_from_kwargs(self, mode:str, file:str,kwargs: Dict, causal: Optional[bool] = False):
            ...
            if file == 'statistics':
                ...
                if 'climate_model' in kwargs:
                    ...
                    coordinates_fname += kwargs['climate_model'] + '_'
                if 'num_ensembles' in kwargs:
                    fname += str(kwargs['num_ensembles']) + '_'
                    coordinates_fname += str(kwargs['num_ensembles']) + '_'# all
                fname += '_'.join(kwargs['variables']) +'_'
                coordinates_fname += '_'.join(kwargs['variables']) +'_'
                if causal:
                    fname += 'causal_'
            ...
            if file == 'statistics':
                ...
                coordinates_fname += mode + '_' + 'coordinates' + '.npy'
            else:
    ...
    def __init__( # inherits all the stuff from Base
        ...
        if num_ensembles == 1:
            ...
            if len(ensemble) == 1:
                ...
                self.ensemble_dirs = [os.path.join(self.root_dir, ensemble[0])] # THIS USED TO BE THE CASE: Taking specific ensemble member (#TODO: only this ensemble member has historical data...)
        else:
            log.warn("Data loader not properly yet implemented for multiple ensemble members, but we are trying something here.")
        ...
        if os.path.isfile(os.path.join(output_save_dir, fname)): # we first need to get the name here to test that...
            ...
            stats_fname, coordinates_fname = self.get_save_name_from_kwargs(mode=mode, file='statistics', kwargs=fname_kwargs)
            stats = self.load_dataset_statistics(os.path.join(self.output_save_dir, stats_fname), mode=self.mode, mips='cmip6')
            self.coordinates = self.load_dataset_coordinates(os.path.join(self.output_save_dir, coordinates_fname), mode=self.mode, mips='cmip6')
            ...
            for var in variables:
                ...
                for exp in scenarios:
                    ...
                    for ensemble_dir in self.ensemble_dirs:
                        ...
                        #for y in self.get_years_list(get_years, give_list=True):
                            ...
                            if len(files) == 0:
                                ...
                                files = glob.glob(var_dir + f'/*.grib', recursive=True)
                        ...
                        print('length of output_nc_files. after looping through years for 1 of the ensemble members:', len(output_nc_files))
                    ...
                    print('Here the final var_dir be:', var_dir)
                    print('length of all_ensemble_output_nc_files after looping through all ensemble members:', len(all_ensemble_output_nc_files))
            ...
            if self.mode == 'train' or self.mode == 'train+val':
                ...
                stats_fname, coordinates_fname = self.get_save_name_from_kwargs(mode='train+val', file='statistics', kwargs=fname_kwargs)
...
class Input4MipsDataset(ClimateDataset):
    ...
    def __init__( # inherits all the stuff from Base
        ...
        if os.path.isfile(os.path.join(output_save_dir, fname)): # we first need to get the name here to test that...
            ...
            self.coordinates = self.load_dataset_coordinates(os.path.join(self.output_save_dir, coordinates_fname), mode=self.mode, mips='input4mips')
            self.Data = self.normalize_data(self.raw_data, stats)
            if self.seasonality_removal:
                self.Data = self.remove_seasonality(self.Data)
            ...
            if self.mode == 'train' or self.mode == 'train+val':
                ...
                stats_fname, coordinates_fname = self.get_save_name_from_kwargs(mode='train+val', file='statistics', kwargs=fname_kwargs) #Load train stats cause we don't calculcate norm stats for test.
