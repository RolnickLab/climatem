...
class TrainingLatent:
    ...
    def train_step(self):  # noqa: C901
        ...
        self.train_loss = nll.item() if self.optim_params.udpate_ALM_using_nll else loss.item()
    ...
    def get_ortho_violation(self, w: torch.Tensor) -> float:
        ...
        if self.iteration > self.optim_params.schedule_ortho:
            # What should be the size here? Is the first dimension the different variables?
        ...
        if y_true.size(-1) == self.lat * self.lon:
            ...
            if take_spherical_harmonics:
                ...
                if take_log:
                    ...
                    idx_pos = torch.abs(alm_true) < 1e-8
                    alm_true = torch.where(idx_pos, 0.0, alm_true)
                    alm_pred = torch.where(idx_pos, 0.0, alm_pred)
                    alm_true = torch.log(torch.abs(alm_true) + 1e-8)
                    alm_pred = torch.log(torch.abs(alm_pred) + 1e-8)
    ...
    def get_temporal_spectral_loss(self, x, y_true, y_pred):
        ...
        # obs = torch.mean(obs, dim=-1, keepdim=True)
