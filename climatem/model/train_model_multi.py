...
This supports n_global_forcings = 2 and n_aerosol_forcings = 2 This module implements the full training pipeline for the
LatentTSDCD causal discovery model, including multi-phase training (encoder/decoder warmup followed by joint
optimization with causal graph learning), ALM-based constrained optimization (orthogonality, sparsity, and acyclicity
constraints), loss computation (ELBO, CRPS, spectral losses, forcing losses), validation, checkpoint saving, and
...
class TrainingLatent:
    ...
    def __init__(
        ...
        self.train_ortho_so2_forcing_cons_list = []
        self.train_ortho_so2_forcing_vector_cons_list = []
        ...
        self.mu_ortho_so2_forcing_list = []
        self.gamma_ortho_so2_forcing_list = []

        self.h_ortho_list = []
        self.h_ortho_aerosol_forcing_list = []
        self.h_ortho_so2_forcing_list = []
        ...
        self.valid_ortho_so2_forcing_cons_list = []
        self.valid_ortho_so2_forcing_vector_cons_list = []
        ...
        with torch.no_grad():
            ...
            if self.latent:
                ...
                if model.use_forced_latents:
                    self.n_climate = self.d_z - model.n_forced_latents_total
                else:
                    self.n_climate = self.d_z
    ...
    def train_with_QPM(self):  # noqa: C901
        ...
        self.ALM_ortho = ALM(
            ...
            dim_gamma=(self.n_climate, self.n_climate),  # Only climate latents
        ...
        self.ALM_ortho_aerosol_forcing = ALM(
            ...
            self.optim_params.ortho_spatial_omega_mu,
            self.optim_params.ortho_spatial_h_threshold,
            self.optim_params.ortho_spatial_min_iter_convergence,
            dim_gamma=(self.model.n_forced_latents_aerosol, self.model.n_forced_latents_aerosol),  # Only BC
            valid_freq=self.train_params.valid_freq,
        )
        #  Spatial forcing SO2
        self.ALM_ortho_so2_forcing = ALM(
            self.optim_params.ortho_spatial_mu_init,
            self.optim_params.ortho_spatial_mu_mult_factor,
            self.optim_params.ortho_spatial_omega_gamma,
            self.optim_params.ortho_spatial_omega_mu,
            self.optim_params.ortho_spatial_h_threshold,
            self.optim_params.ortho_spatial_min_iter_convergence,
            dim_gamma=(self.model.n_forced_latents_so2, self.model.n_forced_latents_so2),  # Only SO2
            valid_freq=self.train_params.valid_freq,
        )
        ...
        while self.iteration < self.train_params.max_iteration and not self.ended:
            ...
            if self.logging_iter > 0 and self.iteration % (self.plot_params.plot_freq) == 0:
                ...
                self.plotter.plot_sparsity(self, multiple_forcings=True)
            ...
            if not self.converged:
                ...
                if self.iteration % self.train_params.valid_freq == 0:
                    ...
                    if self.optim_params.update_ALM_spatial:
                        ...
                        self.ALM_ortho_so2_forcing.update(
                            self.iteration, self.valid_ortho_so2_forcing_vector_cons_list, alm_update_loss_list
                        )
                    ...
                    if self.iteration > 1000:
                        if not self.no_w_constraint:
                            ...
                            if self.optim_params.update_ALM_spatial:
                                ortho_spatial_forcing_converged = (
                                    self.ALM_ortho_aerosol_forcing.has_converged
                                    & self.ALM_ortho_so2_forcing.has_converged
                                )
        ...
        self.plotter.plot_sparsity(self, save=True, multiple_forcings=True)
        self.print_results()
        ...
        valid_loss = {
            ...
            "valid_ortho_so2_forcing_cons": torch.sum(self.valid_ortho_so2_forcing_cons).item(),
    ...
    def train_step(self):  # noqa: C901
        ...
        if isinstance(batch, dict):
            ...
            if y_global_focings is not None and y_spatial_forcings is not None:
                ...
                y_ch4 = y_global_focings[:, :, 1]
                y_so2 = y_spatial_forcings[:, :, 1]
        else:
            x, y = batch
            y_co2, y_aerosol, y_ch4, y_so2 = None, None, None, None
        ...
        if y_ch4 is not None:
            y_ch4 = torch.nan_to_num(y_ch4)
        if y_so2 is not None:
            y_so2 = torch.nan_to_num(y_so2)
        if self.iteration == 1:
            print(
                f"Dataloader x shape {x.shape}, y shape {y.shape}, y_global_focings shape {y_global_focings.shape}, y_spatial_forcings shape {y_spatial_forcings.shape}"
            )
        ...
        for k in range(self.future_timesteps):
            ...
            if not self.instantaneous:
                y_pred, y_spare, z_spare, pz_mu, pz_std = self.model.predict(
                    x_bis, y[:, k], y_co2=y_co2, y_aerosol=y_aerosol, y_ch4=y_ch4, y_so2=y_so2
                )
            else:
        ...
        h_ortho_aerosol_forcing = self.get_ortho_violation_forcings(
            self.model.autoencoder.get_w_forcings("aerosol"), self.ortho_aerosol_normalization
        )
        h_ortho_so2_forcing = self.get_ortho_violation_forcings(
            self.model.autoencoder.get_w_forcings("so2"), self.ortho_so2_normalization
        )
            ...
            if self.constraint_func == "sum":
                ...
                if self.optim_params.update_ALM_spatial:
                    loss = (
                        ...
                        + torch.sum(self.ALM_ortho_so2_forcing.gamma @ h_ortho_so2_forcing)
                        + 0.5 * self.ALM_ortho_so2_forcing.mu * torch.sum(h_ortho_so2_forcing**2)
                    )
        ...
        for k in range(self.future_timesteps):
            ...
            px_mu, px_std = self.model.predict_pxmu_pxstd(
                torch.cat((x[:, k:], y_pred_all[:, :k]), dim=1),
                y[:, k],
                y_co2=y_co2,
                y_aerosol=y_aerosol,
                y_ch4=y_ch4,
        ...
        if self.model.autoencoder.use_grad_project and not self.no_w_constraint:
            with torch.no_grad():
                if self.model.use_forced_latents:
                    n_climate = self.model.d_z - self.model.n_forced_latents_total
                else:
                    n_climate = self.model.d_z
                self.model.autoencoder.get_w_decoder()[..., :n_climate].clamp_(min=0.0)
                self.model.autoencoder.get_w_forcings("co2").clamp_(min=0.0)
                self.model.autoencoder.get_w_forcings("aerosol").clamp_(min=0.0)
                self.model.autoencoder.get_w_forcings("ch4").clamp_(min=0.0)
                self.model.autoencoder.get_w_forcings("so2").clamp_(min=0.0)
        ...
        self.train_ortho_so2_forcing_cons = h_ortho_so2_forcing.detach()
        ...
        self.train_forcing_ch4_loss = forcing_loss_ch4.item()
        self.train_forcing_so2_loss = forcing_loss_so2.item()
        ...
        if self.iteration % 10 == 0 and self.model.use_forced_latents:
            logger.info(
                ...
                f"Aerosol BC={forcing_loss_aerosol.item():.6f}, "
    ...
    def valid_step(self):  # noqa: C901  # noqa: C901
        ...
        with torch.no_grad():
            ...
            self.valid_ortho_so2_forcing_cons = h_ortho_so2_forcing.detach()
        ...
        if self.iteration % self.plot_params.print_freq == 0:
            ...
            print(f"Iteration {self.iteration}, Ortho_so2 constraint sum {h_ortho_so2_forcing}")
            np.save(
            ...
            np.save(
                self.save_path / f"decoder_aerosol_mat_{self.iteration}.npy",
                self.model.autoencoder.get_w_aerosol().cpu().detach().numpy(),
            )
            np.save(
                self.save_path / f"decoder_so2_mat_{self.iteration}.npy",
                self.model.autoencoder.get_w_so2().cpu().detach().numpy(),
            )
    ...
    def log_losses(self):
        ...
        self.train_ortho_so2_forcing_cons_list.append(torch.sum(self.train_ortho_so2_forcing_cons))
        self.train_ortho_so2_forcing_vector_cons_list.append(self.train_ortho_so2_forcing_cons)
        ...
        self.valid_ortho_so2_forcing_cons_list.append(torch.sum(self.valid_ortho_so2_forcing_cons))
        self.valid_ortho_so2_forcing_vector_cons_list.append(self.valid_ortho_so2_forcing_cons)
        ...
        self.mu_ortho_so2_forcing_list.append(self.ALM_ortho_so2_forcing.mu)
        self.gamma_ortho_so2_forcing_list.append(self.ALM_ortho_so2_forcing.gamma)
    ...
    def get_nll(
        ...
        if (
            ...
            logger.info("Real climate data has no GT Latents")
    ...
    def get_ortho_violation(self, w: torch.Tensor, norm_factor) -> float:  # [1, d_x, d_z]
    ...
    def get_ortho_violation_forcings(self, w: torch.Tensor, norm_factor) -> float:

        if self.iteration > self.optim_params.schedule_ortho:
            # print("w", w.shape)#(400, 2) or (400,1)

            k = w.size(-1)

            constraint = w.T @ w - torch.eye(k, device=w.device)
            h = constraint / norm_factor
        else:
            h = torch.as_tensor([0.0])
    ...
    def autoregress_prediction_original(self, valid: bool = False, timesteps: int = 120):
        ...
        if not valid:
            ...
            if isinstance(batch, dict):
                x = batch["x"]
                y = batch["y"]
                y_global_focings = batch["global_forcings"]
                y_spatial_forcings = batch["spatial_forcings"]
                y_co2 = y_global_focings[:, :, 0]
                y_aerosol = y_spatial_forcings[:, :, 0]
                y_ch4 = y_global_focings[:, :, 1]
                y_so2 = y_spatial_forcings[:, :, 1]
            ...
            with torch.no_grad():
                y_pred, y, z, pz_mu, pz_std = self.model.predict(
                    x, y, y_co2=y_co2, y_aerosol=y_aerosol, y_ch4=y_ch4, y_so2=y_so2
                )

                # Here we predict, but taking 100 samples from the latents
                # TODO: make this into an argument
                samples_from_xs, samples_from_zs, y = self.model.predict_sample(
                    x, y, 10, y_co2=y_co2, y_aerosol=y_aerosol, y_ch4=y_ch4, y_so2=y_so2
                )
            ...
            with torch.no_grad():
                ...
                samples_from_xs, samples_from_zs, y = self.model.predict_sample(
                    x, y, 100, y_co2=y_co2, y_aerosol=y_aerosol, y_ch4=y_ch4, y_so2=y_so2
                )
    ...
    def particle_filter(self, x, y, num_particles, timesteps=120, y_co2=None, y_aerosol=None, y_ch4=None, y_so2=None):
        ...
        for _ in range(timesteps):
            ...
            _, samples_from_zs, y = self.model.predict_sample(
                x, y, 100, y_co2=y_co2, y_aerosol=y_aerosol, y_ch4=y_ch4, y_so2=y_so2
            )
