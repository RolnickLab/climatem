...
class TrainingLatent:
    def __init__(self, model, datamodule, hp, best_metrics, d, profiler = False, profiler_path = './log'):
        ...
        if self.instantaneous:
            self.adj_tt = torch.zeros([int(self.hp.max_iteration / self.hp.valid_freq), self.tau + 1,
                                    self.d * self.d_z, self.d * self.d_z])
        else:
            self.adj_tt = torch.zeros([int(self.hp.max_iteration / self.hp.valid_freq), self.tau, self.d *
    ...
    def train_with_QPM(self):
        ...
        self.ALM_sparsity = ALM(self.hp.sparsity_mu_init,
        ...
        if self.profiler:
            ...
            def trace_handler(p):
                ...
                print(output_cpu)
                print(output_cuda)
                p.export_chrome_trace("/home/mila/s/sebastian.hickman/scratch/profiling/" + str(p.step_num) + ".json")
            
            prof = torch.profiler.profile(
                activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                schedule=torch.profiler.schedule(wait=5, warmup=5, active=1, repeat=1),
                # using the torch tensorboard handler
                on_trace_ready=torch.profiler.tensorboard_trace_handler('/home/mila/s/sebastian.hickman/scratch/profiling'),
        ...
        while self.iteration < self.hp.max_iteration and not self.ended:
            ...
            if self.iteration % self.hp.valid_freq == 0:
                ...
                if self.iteration % (self.hp.print_freq) == 0:
                    ...
                    accelerator.log({"kl_train": self.train_kl, "loss_train": self.train_loss, "recons_train": self.train_recons,
                          "kl_valid": self.valid_kl, "loss_valid": self.valid_loss, "recons_valid": self.valid_recons,
                          "nll_train": self.train_nll, "nll_valid": self.valid_nll,
                          "mae_recons_train": self.train_mae_recons, "mae_pred_train": self.train_mae_pred, "mae_persistence_train": self.train_mae_persistence,
                          "mae_recons_valid": self.val_mae_recons, "mae_pred_valid": self.val_mae_pred,
                          "mse_recons_train": self.train_mse_recons, "mse_pred_train": self.train_mse_pred,
                          "mse_recons_valid": self.val_mse_recons, "mse_pred_valid": self.val_mse_pred,
                          "var_original_train": self.train_var_original, "var_recons_train": self.train_var_recons, "var_pred_train": self.train_var_pred,
                          "var_original_valid": self.val_var_original, "var_recons_val": self.val_var_recons, "var_pred_valid": self.val_var_pred, 
                          
                          "mae_recons_valid_1": self.val_mae_recons_1, "mae_recons_valid_2": self.val_mae_recons_2, "mae_recons_valid_3": self.val_mae_recons_3, "mae_recons_valid_4": self.val_mae_recons_4,
                          "mae_pred_valid_1": self.val_mae_pred_1, "mae_pred_valid_2": self.val_mae_pred_2, "mae_pred_valid_3": self.val_mae_pred_3, "mae_pred_valid_4": self.val_mae_pred_4,                                     
            ...
            if self.logging_iter > 0 and self.iteration % (self.hp.plot_freq) == 0:
                ...
                torch.save(self.model.state_dict(), self.hp.exp_path + '/model.pth')     
            ...
            if not self.converged:
                ...
                if self.iteration % self.hp.valid_freq == 0:
                    ...
                    self.ALM_sparsity.update(self.iteration,
                                            self.valid_sparsity_cons_list,
                                            self.valid_loss_list)
    ...
    def train_step(self):
        ...
        if self.model.module.autoencoder.use_grad_project and not self.no_w_constraint:
            with torch.no_grad():
                self.model.module.autoencoder.get_w_decoder().clamp_(min=0.)
            assert torch.min(self.model.module.autoencoder.get_w_decoder()) >= 0.
        ...
        if self.iteration % self.hp.print_freq == 0:
            ...
            np.save(self.hp.exp_path+'/x_true_recons_train.npy', x.cpu().detach().numpy())
            np.save(self.hp.exp_path+'/y_true_recons_train.npy', y.cpu().detach().numpy())
            np.save(self.hp.exp_path+'/y_pred_recons_train.npy', y_pred_recons.cpu().detach().numpy())

            # also carry out autoregressive predictions
            mse, smape, y_original, y_original_pred, y_original_recons, x_original = self.autoregress_prediction_original(valid=False, timesteps=10)
            ...
            if self.d == 1 or self.d == 4:
                self.plotter.plot_compare_predictions_icosahedral(x_past=x_original[:, -1, :, :].cpu().detach().numpy(), 
                                                                ...
                                                                sample=np.random.randint(0, self.batch_size), coordinates=self.coordinates,
                ...
                self.plotter.plot_compare_predictions_icosahedral(x_past=x_original[:, -1, :, :].cpu().detach().numpy(), 
                                                                ...
                                                                y_recons=y_original_recons.cpu().detach().numpy(), 
                                                                y_hat=y_original_pred.cpu().detach().numpy(), 
                                                                sample=np.random.randint(0, self.batch_size), coordinates=coordinates,
                                                                path=self.hp.exp_path, iteration=self.iteration, 
                                                                valid=False, plot_through_time=True)
    ...
    def valid_step(self):
        ...
        if self.iteration % self.hp.print_freq == 0:
            ...
            np.save(self.hp.exp_path+'/x_true_recons_val.npy', x.cpu().detach().numpy())
            np.save(self.hp.exp_path+'/y_true_recons_val.npy', y.cpu().detach().numpy())
            np.save(self.hp.exp_path+'/y_pred_recons_val.npy', y_pred.cpu().detach().numpy())
            ...
            if self.d == 1 or self.d == 4:
                self.plotter.plot_compare_predictions_icosahedral(x_past=x_original[:, -1, :, :].cpu().detach().numpy(), 
                                                                ...
                                                                sample=np.random.randint(0, self.batch_size), coordinates=coordinates,
                                                                path=self.hp.exp_path, iteration=self.iteration, 
                                                                valid=True, plot_through_time=True)
    ...
    def get_crps_loss(self, y, mu, sigma):
        ...
        We need to extract the parameters of the Gaussian of the model.
        I am going to start by just taking the parameters of all the Gaussians for the observations first...
        ...
        pi_inv = 1. / torch.sqrt(torch.tensor([np.pi]))
    ...
    def get_spectral_loss(self, y_true, y_pred):
        """
        Calculate the spectral loss between the true values and the predicted values.
        We need to calculate the spectra of thhe true values and the predicted values, and then determine an appropriate metric to compare them.
    ...
    def autoregress_prediction_original(self, valid: bool = False, timesteps: int = 120):
        ...
        We also do an autoregressive lead out for set number of timesteps, with a default of 120 timesteps of rollout.
